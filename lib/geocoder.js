...
Geocoder.prototype.geocode = function(location, callback) {
	...
	http.get(Geocoder.endpoints.PRIMARY + '?' + qs, function(result) {
		...
		self._processHttpResult.call(self, result, callback);
	...
	var self = this,
		...
		stream = sax.createStream(false, saxOpts)
			...
			.on('text', function(text) {
				switch (currentNode) {
					case 'latt':
						self.coords.longitude = parseFloat(text);
						break;
					case 'longt':
						self.coords.latitude = parseFloat(text);
						break;
				}
			})
			.on('end', function() {
				if (self.coords.longitude && self.coords.latitude) {
					this.emit("result", self.coords);
					callback.call(self.coords, null, self.coords);
				} else {
